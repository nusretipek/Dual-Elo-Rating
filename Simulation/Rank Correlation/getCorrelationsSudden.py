...
def hierarchy_by_time(t: int) -> List[Sequence[int]]:
    # tiers are best -> worst
    if 0 <= t < 200:
        return [[0], [4, 1], [6, 5, 2], [3]]
    if 200 <= t < 400:
        return [[0], [4, 2], [6, 5, 1], [3]]
    if 400 <= t < 600:
        return [[4], [0, 2], [6, 5, 1], [3]]
    return [[4], [0, 2], [6, 1], [5], [3]]
...
def compute_avg_spearman(series: List[np.ndarray], T: int) -> Tuple[float, Dict[str, float]]:
    ...
    interval_avgs = {k: round(float(np.nanmean(np.array(v, dtype=float))), 3) if v else float("nan")
...
def main():
    ...
    print("=== Spearman rank correlation vs true tiered hierarchy ===")
